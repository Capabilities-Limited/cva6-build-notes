...
```
(also install `vim`: `sudo apt install -y vim`)
...
To view the test log, look in the dated directory out_ here: ~/cva6/verif/sim/out_*/iss_regr.log

Note that the log should show the comparison between tests running the
CVA6 core in Verilator against the Spike ISA simulator.
